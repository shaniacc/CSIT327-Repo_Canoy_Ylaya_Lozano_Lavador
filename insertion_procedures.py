...
def update_student_procedure(student_id ,first_name , last_name , roll_number ):
    try:
        ...
        db.session.execute(sql_query,{"student_id": student_id, "first_name": first_name, "last_name": last_name, "roll_number": roll_number})
        db.session.commit()
    except Exception as e:
        print(f"Error as {e}")
        db.session.rollback()
...
def update_course_procedure(course_id, course_name, course_code,start_time, end_time ):
    try:
        sql_query = text("CALL UpdateCourse(:course_id, :course_name, :course_code, :start_time, :end_time)")
        db.session.execute(sql_query,{"course_id": course_id, "course_name": course_name, "course_code": course_code, "start_time": start_time, "end_time": end_time})
        db.session.commit()
    except Exception as e:
        print(f"Error as {e}")
        db.session.rollback()
